# Eigenvalue Restriction Lemma: From Boolean Action to PVM Structure

**Date:** 2026-03-12
**Status:** Technical Development (addressing Core Derivation Step 5)
...
where $D = L_3(I_\infty)$ is the domain of L₃-admissible configurations.
...
$$p(E|\psi) : \text{Events} \times \text{States} \to [0,1]$$

Given state $|\psi\rangle$, $p(E|\psi)$ is the probability that E obtains upon measurement.
...
**Events** are Boolean propositions: "spin is up", "particle detected in region A". Under L₃, every event is determinately true or false of an actual configuration.
...
**Premise 1 (From L₃):** For any well-formed proposition P about an actual configuration $c \in A_\Omega$:
- P = P (Identity)
...
$(\Leftarrow)$ Suppose $P^2 = P$. Then for any eigenvalue $\lambda$ with eigenvector $|\phi\rangle$:
...
### 3.4 Synthesis: Boolean A → PVM-Only Events

Combining Conclusions 1-3:

1. A's Boolean character (from L₃) entails Boolean actualization values
...
### 4.2 The L₃ Argument Against Primitive POVMs

Consider a POVM element $E$ with eigenvalue $\lambda \in (0,1)$ and corresponding eigenstate $|\phi\rangle$.

**The problematic question:** Is event E actual for system in state $|\phi\rangle$?

The formalism gives:
$$\langle\phi|E|\phi\rangle = \lambda \in (0,1)$$
...
**(a) λ is a probability, not an actualization value:**
This works: $\lambda$ represents $p(E|\phi)$, the probability of E given state $|\phi\rangle$. But then E is not a predicate about the actual configuration—it is a statistical summary of possible outcomes.

**(b) The eigenstate condition is unphysical:**
Perhaps $|\phi\rangle$ (eigenstate of non-projection $E$) cannot be an L₃-admissible configuration. Then $|\phi\rangle \notin A_\Omega$, and the problematic case doesn't arise.
...
**Objection:** Quantum logic (Birkhoff-von Neumann) replaces distributivity with a weaker lattice structure. LRT's L₃ imposes classical logic where quantum logic would be more appropriate.

**Response:** LRT applies L₃ at the **vehicle level**—completed measurement records—not at the content level (quantum state superpositions). At the vehicle level, every record is Boolean: the pointer is at position x or it isn't. Quantum logic describes relationships among propositions in $I_\infty$; L₃ constrains the structure of $A_\Omega$. No conflict.
...
2. Actualization events have Boolean truth values (from L₃'s Excluded Middle)
...
The argument is complete as a philosophical derivation. The chain from L₃ → Boolean actualization → eigenvalue restriction → projection operators is logically valid.
...
**Dependency check:** This argument does not presuppose Step 3 (local tomography) or Step 4 (Hilbert space). It applies to *any* event-based quantum theory with Boolean actualization. The Hilbert space context is imported for definiteness, but the core argument (Boolean → projection) is algebraic.

---

## 8. Connection to Core Derivation

This document addresses **Critical Conjecture 2** from the Core Derivation (Step 5).
