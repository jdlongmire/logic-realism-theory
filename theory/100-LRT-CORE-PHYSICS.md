...
<!-- MMR-54 revision: Abstract restructured to Background → Method → Results → Conclusion -->
...
<!-- MMR-54 revision: §1.1 and §1.2 compressed to ~1 page total -->
...
<!-- MMR-54 revision: New §1.3.1 inserted — PPC derivation defending L₃ → operational distinguishability -->
...
The argument proceeds in two steps, decomposing the inference into individually defensible claims. The framework premise is that LRT treats L₃ as constitutive rather than regulative. A regulative principle governs reasoning about a domain that exists independently of it; a constitutive principle defines what counts as an element of the domain. Under the constitutive reading, Identity is the condition under which anything is a determinate thing, Non-Contradiction the condition under which description is possible, Excluded Middle the condition under which a claim about a configuration has a truth value. A configuration in $A_\Omega$ is not a pre-existing entity that L₃ then filters; L₃ is what makes it a configuration at all.

**Step 1: From distinctness to informational distinguishability.** Each law of L₃ imposes a content requirement. Identity demands that the property $P$ ascribes makes a difference to $c$'s character. Non-Contradiction demands that $P$-true and $P$-false be genuinely distinct states of affairs. Excluded Middle demands that $c$'s character determines which obtains. All three converge: *there must be a difference between the state of affairs in which $P$ is true and that in which $P$ is false.* Because $I_\infty$ is an information space — its elements are distinguished precisely by their informational content — this difference is an informational difference. Distinct configurations differ in at least one event $E$: there exists $E$ such that $E(c_1) \neq E(c_2)$. This is analytic given the nature of $I_\infty$: a space of information configurations whose elements lack informational individuation is a contradiction in terms.

What Step 1 does not establish: that the informational difference is physically accessible. Two elements of $I_\infty$ that are informationally distinct might, in principle, be physically indistinguishable if only some informational differences have physical consequences.

**Step 2: From informational to operational distinguishability in $A_\Omega$.** Actualization is what separates $A_\Omega$ from $I_\infty$. Both spaces contain informationally distinct configurations. What actualization adds is *physical standing*: the informational content *obtains* in the physical world. If two actualized configurations $c_1, c_2 \in A_\Omega$ are informationally distinct (Step 1: they differ in some event $E$) but operationally indistinguishable (no physically admissible measurement $M$ yields different statistics), then their informational difference makes no physical difference. But actualization is precisely the operator that confers physical standing on informational content. Content that is actualized yet physically inert is a contradiction: it would render the distinction between $A_\Omega$ and $I_\infty$ vacuous.

The operational distinguishability metric formalizes this:

$$D(s_1, s_2) \;=\; \sup_M \;\lvert P_M(s_1) - P_M(s_2)\rvert_{\mathrm{TV}}$$

where $M$ ranges over all physically admissible measurements. For distinct actualized configurations, $D > 0$. We name this result **Operational Determinacy.**

**Combining the steps:** L₃-satisfaction requires that $P$-true and $P$-false be distinct states of affairs (constitutive premise). Distinct states in $I_\infty$ are informationally distinguishable (Step 1). Informationally distinguishable states in $A_\Omega$ are operationally distinguishable (Step 2). Therefore L₃-satisfaction requires operational distinguishability. Operational Determinacy is not a new axiom but a consequence of applying L₃'s constitutive requirements through the informational structure of $I_\infty$ and the physical standing conferred by actualization.

Two objections require direct response. The **verificationism charge** holds that the PPC covertly assumes propositions must be verifiable to be meaningful. The claim here is different in kind: not that operationally inaccessible propositions lack meaning, but that configurations without any operational signature are not configurations in $A_\Omega$ at all. The constraint is ontological — what counts as a physical state of affairs — not epistemological. The charge targets Step 2, and the critic must provide a non-vacuous account of actualization that confers no physical consequence. The **equivocation charge** holds that "distinct states of affairs" (logical) is silently swapped for "operationally distinguishable" (physical). The two-step decomposition makes the bridge explicit: Step 1 moves from logical distinctness to informational distinguishability (via $I_\infty$'s nature); Step 2 moves from informational to operational distinguishability (via $A$'s nature). Each step has its own grounds and can be challenged independently. Full development appears in Supplement S1.

The Bohmian case deserves specific note. Bohmian mechanics does not actually deny Step 2: Bohmian positions *do* have operational consequences (they determine detection events). The Bohmian dispute is about whether positions are always determinate, not about whether determinate positions are operationally inert. LRT and Bohmian mechanics agree on Step 2; they disagree about the scope of determinacy, which is a different argument.

This subsection establishes that the PPC is derived, not stipulated: L₃-satisfaction, given the constitutive reading, entails informational distinguishability (Step 1), which in the actualized domain entails operational distinguishability (Step 2). What it leaves open is whether a critic who rejects L₃'s constitutive status can be compelled to accept Operational Determinacy on independent grounds. That is a foundational disagreement about the nature of logic, not a gap in the present derivation.
...
<!-- MMR-54 revision: §1.5 and §1.6 merged into single "Scope and Limits" section -->
...
<!-- MMR-54 revision: Operational Determinacy Bridge and Separation Step inserted from draft_separation_lemma.md -->
...
<!-- MMR-54 revision: Supplement reference to archive bridge work added -->
*[Note: Extended treatment of the H1→H2 bridge argument, including the formal separation proof sketch and dependency graph, appears in theory/supplementary/ (H1_H2_Bridge_Archive).]*

<!-- MMR-54 revision: New §3.1.1 inserted — countermodel response addressing real-QT, Spekkens, Renou et al. -->
...
<!-- MMR-54 revision: "derives" → "reconstructs given" for imported theorem dependence -->
...
<!-- MMR-54 revision: "derives" → "reconstructs given" — Gleason is imported, not originated -->
...
<!-- MMR-54 revision: "derive" → "reconstruct given" throughout §5 -->
...
<!-- MMR-54 revision: §7.1 comparison table replaced with feature matrix per Issue #52 -->
...
The following matrix compares LRT against the principal operational reconstruction programs on nine criteria central to quantum foundations. Entries marked ✓ indicate the feature is present; ✗ indicates absent; and qualifications are noted where the situation is more complex.
...
<!-- MMR-54 revision: Philosophy of QM citations added — Ladyman (2007), Wallace (2012), Timpson (2013) -->
...
<!-- MMR-54 revision: Philosophy of QM references added -->
...
*Logic Realism Theory Project | Last Updated: 2026-03-23*
