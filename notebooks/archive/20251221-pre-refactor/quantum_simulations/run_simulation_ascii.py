...
ASCII-only version for Windows terminal.
"""
...
try:
    ...
    print("OK Qiskit imports successful")
except ImportError as e:
    print(f"ERROR: Qiskit import error: {e}")
...
print(f"   T1: {noise_config.t1*1e6:.1f} us")
print(f"   T2: {noise_config.t2*1e6:.1f} us")
...
print(f"   OK Simulator configured: {simulator.name}")
...
print(f"   OK Low-entropy circuit: {low_qc.count_ops()}")
print(f"   OK High-entropy circuit: {high_qc.count_ops()}")
...
print(f"   Low-entropy (with noise): Delta_S = {Delta_S:.6f} nats")
...
print(f"\n   Model: log(p_log) = alpha + beta * Delta_S")
...
try:
    ...
    ax2.plot(x_fit, y_fit, 'r-', linewidth=2,
            label=f'Fit: log(p) = {alpha:.2f} + {beta:.2f}*Delta_S')
...
if beta > 0 and p_value < 0.05:
    print(f"\n  OK POSITIVE RESULT: Evidence for entropy-error correlation")
